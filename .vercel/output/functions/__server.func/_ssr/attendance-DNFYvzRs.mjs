...
import { y as LayoutGrid, a8 as CalendarDays, l as CircleAlert, j as CircleCheck, a9 as LogIn, i as LogOut, e as Users, p as ChevronLeft, r as ChevronRight, D as Download, C as Clock, aa as Timer, k as CircleX, a as LoaderCircle } from "../_libs/lucide-react.mjs";
...
function AttendancePage() {
  ...
  const submitManualAttendance = async () => {
    ...
    setBusy(true);
    const inTime = (/* @__PURE__ */ new Date(`${manualDate}T${manualCheckIn}:00+05:30`)).toISOString();
    const outTime = manualCheckOut ? (/* @__PURE__ */ new Date(`${manualDate}T${manualCheckOut}:00+05:30`)).toISOString() : null;
    const {
      error
    } = await supabase.from("attendance").upsert({
  ...
  const handleExport = async () => {
    ...
    const rows = attendanceData.map((record) => {
      ...
      return [record.date, `"${empName}"`, `"${empEmail}"`, status, `"${inTime}"`, `"${outTime}"`, `"${duration}"`].join(",");
    });
